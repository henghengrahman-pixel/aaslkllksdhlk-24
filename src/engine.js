...
import { runConversationAgent, validateAgentDecision, agentDecisionToLegacy } from './ai-agent.js';
...
async function executeConversationAgentTurn({chatId,eventId,threadId=null,text,normalized,intent,attachments=[],livechat,beforeState,src=null}){
  ...
  const memberIdForAgent=currentCaseUserId(text,workflowForAgent,recentForAgent)||extractUserId(recentForAgent)||'';
  ...
  const previousIntent=String(sources.caseBrain?.primary_intent||workflowIntent||stateForAgent?.last_topic||'').toUpperCase();
  const currentIntent=String(intent||'GENERAL').toUpperCase();
  const meaningfulPrevious=previousIntent && !['GENERAL','GREETING','UNKNOWN','NONE','NEW'].includes(previousIntent) ? previousIntent : '';
  const meaningfulCurrent=currentIntent && !['GENERAL','GREETING','UNKNOWN','NONE','NEW'].includes(currentIntent) ? currentIntent : '';
  const activeCaseForAgent=workflowIntent||meaningfulPrevious||meaningfulCurrent;
