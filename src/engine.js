...
async function sendAndStore(livechat,chatId,text,intent,senderType='ai'){
  ...
  try{
    ...
    else err.retryable=true; // delivery/network/provider failure is recovered by durable ingress
...
async function executeConversationAgentTurn({chatId,eventId,threadId=null,text,normalized,intent,attachments=[],livechat,beforeState,src=null}){
  ...
  const ticketForAgent=await db.getOpenBridgeTicketByChat(chatId).catch(()=>null);
  ...
  const previousIntent=String(sources.caseBrain?.primary_intent||workflowIntent||beforeState?.last_topic||'').toUpperCase();
  ...
  const agentInput={
    ...
    state:agentState,currentIntent:intent,previousIntent,activeCase:workflowIntent||previousIntent||intent,
