const typoMap = new Map(Object.entries({
  'blm':'belum','blom':'belum','belom':'belum','blumm':'belum','bloom':'belum',
  'msk':'masuk','msuk':'masuk','masok':'masuk','msokk':'masuk','masokk':'masuk',
  'wd':'withdraw','wede':'withdraw','withdrw':'withdraw','widraw':'withdraw','wdraw':'withdraw','witdrow':'withdraw','witdraw':'withdraw','widhraw':'withdraw','penarikn':'withdraw','cashout':'withdraw','cairin':'withdraw',
  'dp':'deposit','dpo':'deposit','dps':'deposit','depo':'deposit','depoo':'deposit','depsit':'deposit','dposit':'deposit','depsoit':'deposit','depost':'deposit','depoist':'deposit','topup':'deposit',
  'psw':'password','pwd':'password','lupapsw':'lupa password','pw':'password','pass':'password','paswod':'password','pasword':'password','passwordd':'password','sandi':'password','sndi':'password','sandi':'password','sandiku':'password','sandy':'password',
  ...
  'gmn':'gimana','gmna':'gimana','knp':'kenapa','kpn':'kapan','skrg':'sekarang','tdk':'tidak','klw':'kalau','klo':'kalau','krn':'karena','karna':'karena','trs':'terus','trus':'terus','mksd':'maksud','mksdnya':'maksudnya','batalin':'batalkan','cancelin':'batalkan','brcd':'barcode','qrnya':'qr','qrisnya':'qris','slmt':'selamat','smt':'selamat','mlm':'malam','malem':'malam','pgi':'pagi','pg':'pagi','sre':'sore','sor':'sore','sng':'siang','gngguan':'gangguan','gangguanx':'gangguan','limid':'limit','limitt':'limit','eror':'error','err':'error','errorr':'error','rolingan':'rollingan','rolllingan':'rollingan','rolinggan':'rollingan','rolling':'rollingan','loginn':'login','ligin':'login','lgin':'login','lupa':'lupa','lpa':'lupa','lp':'lupa','lup':'lupa','alihin':'alihkan','alihkn':'alihkan','rek':'rekening',
...
export function normalizeText(text='') {
  return String(text)
    ...
    .replace(/\blupa\s+(?:pass|sandi)\b/g, 'lupa password')
    .replace(/\bisi\s+saldo\b/g, 'deposit')
    .replace(/\bsetor(?:an)?\b/g, 'deposit')
    .replace(/\btarik\s+dana\b/g, 'withdraw')
    ...
    .map(w => {
      ...
      if (/^(?:blm|blom|belom|blumm)\d*$/.test(w)) return 'belum';
      if (/^(?:msk|msuk|msokk|masokk|masuk)\d*$/.test(w)) return 'masuk';
