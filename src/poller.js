...
async function processChatSnapshot(livechat,chat,summary=null,rank=null,{force=false}={}){
  ...
  const state={...sourceState,active:(summary&&sourceState.active==null)?true:sourceState.active,rank};
...
async function runDeepSync(livechat){
  ...
  try{
    ...
    for (const [rank, summary] of chats.entries()) {
      ...
      const rawState=livechat.chatState(summary);const state={...rawState,active:rawState.active ?? true,rank}; await db.upsertConversation(summary,{visible:true,state});
    ...
    await runConcurrent(missing,Math.min(config.lcSyncConcurrency,6),async(id)=>{
      try{
        ...
        if(st.active===true) await db.upsertConversation(detail,{visible:true,state:st});
      }catch(e){
...
export async function syncOnce(livechat,{manual=false}={}){
  ...
  try{
    ...
    for (const [rank,summary] of chats.entries()){
      ...
      try{
        const rawState=livechat.chatState(summary);const state={...rawState,active:rawState.active ?? true,rank};
