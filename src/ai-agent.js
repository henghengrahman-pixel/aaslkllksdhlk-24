...
const SESSION_CASE_INTENTS=new Set([
  ...
  'GAME_PROBLEM','GENERAL_DISTURBANCE','LOSS_COMPLAINT','TRANSACTION_AMBIGUOUS'
]);
...
export function inferSessionCaseFromHistory(rows=[], {currentText='',currentIntent='GENERAL'}={}){
  ...
  const hasProblem=/\b(?:belum|blm|belom|lama|pending|gagal|error|eror|masalah|kendala|tidak\s+masuk|ga\s+masuk|gak\s+masuk|cek|proses)\b/i.test(n);
