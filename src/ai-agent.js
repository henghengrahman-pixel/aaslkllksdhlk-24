...
export function contextualClarification(input={}){
  const active=String(input.activeCase||input.previousIntent||'').trim().toUpperCase();
  // Fresh sessions and generic/greeting turns do not have an active operational case.
  // Never imply that an old problem exists when the current session has no case context.
  if(!active || ['GENERAL','GREETING','UNKNOWN','NONE','NEW'].includes(active)){
